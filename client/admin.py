...
class HelpTab(QWidget):
    def __init__(self):
        ...
        text.setHtml("""
        <style>
            h1 { color: #42a5f5; font-size: 24pt; margin-bottom: 16px; }
            h3 { color: #90caf9; font-size: 16pt; margin-top: 18px; margin-bottom: 6px; }
            p, li { font-size: 12pt; line-height: 1.45; }
            code { background:#222; padding:2px 6px; border-radius:6px; }
        </style>
        <h1>Biometry Admin — Справка</h1>

        <h3>Пайплайн доступа (door-agent + gateway)</h3>
        <ol>
          <li><b>Presence</b> — в кадре стабильно обнаружено лицо (2 подряд кадра).</li>
          <li><b>Liveness</b> — проверка живости по клипу (multi-frame).</li>
          <li><b>Face ID</b> — поиск ближайшего пользователя + строгие пороги.</li>
          <li><b>Voice ID</b> — дополнительная модальность (если есть речь).</li>
          <li><b>Policy</b> — проверка прав в комнату/зону.</li>
          <li><b>Decision</b> — Доступ/Отказ + причина этапа.</li>
        </ol>

        <h3>Тайминги и как настраивать</h3>
        <ul>
          <li><code>DOOR_MAX_CLIP_FRAMES</code> — сколько кадров door-agent собирает перед проверкой (по умолчанию 3).</li>
          <li><code>DOOR_PRESENCE_TIMEOUT_MS</code> — когда presence-сессия сбрасывается без лица (по умолчанию 1500ms).</li>
          <li><code>DOOR_COOLDOWN_MS</code> — пауза после решения перед новой проверкой (по умолчанию 2000ms).</li>
          <li><code>VISION_RPC_TIMEOUT_MS</code> / <code>AUDIO_RPC_TIMEOUT_MS</code> — таймауты вызовов backend сервисов (по умолчанию 1500/2000ms).</li>
        </ul>

        <h3>Антиспуф и пороги качества</h3>
        <ul>
          <li><code>ACCESS_MIN_FACE_LIVENESS</code> (default 0.90)</li>
          <li><code>ACCESS_CLIP_MIN_MOTION</code> (default 0.012)</li>
          <li><code>ACCESS_CLIP_MIN_MEDIAN_LIVENESS</code> (default 0.55)</li>
          <li><code>ACCESS_CLIP_MIN_LIVE_RATIO</code> (default 0.34)</li>
          <li><code>ACCESS_CLIP_MIN_DETECTED_RATIO</code> (default 0.50)</li>
          <li><code>ACCESS_MIN_SPEECH_SECONDS</code> (default 0.60)</li>
        </ul>

        <h3>Face matching и итоговое решение</h3>
        <ul>
          <li><code>FACE_SIMILARITY_THRESHOLD</code>, <code>FACE_MATCH_MIN_MARGIN</code>, <code>ACCESS_STRICT_FACE_DISTANCE</code></li>
          <li><code>ACCESS_MIN_CONFIDENCE</code> — минимальная итоговая уверенность для выдачи доступа.</li>
        </ul>

        <h3>Где менять значения</h3>
        <p>Для Docker: в <code>docker-compose.yml</code> и/или в <code>.server_runtime.env</code>, затем перезапуск через <code>./start_docker.sh</code>.</p>
        """)
